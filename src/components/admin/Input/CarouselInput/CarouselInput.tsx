...
export default function CarouselInput() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        alert("✅ Carousel item berhasil ditambahkan!");
        router.push("/dashboard/Carousel");
      } else {
        setError(result.message || "Gagal menambahkan carousel item");
      }
    } catch (err) {
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-md">
      <h2 className="text-2xl font-bold text-gray-800 mb-6">Tambah Carousel Item</h2>
