...
case "$1" in
    ...
    logs)
        ...
        ;;
    logs-app)
        docker-compose -f $DOCKER_COMPOSE_FILE -p $PROJECT_NAME logs -f app
        ;;
    logs-db)
    ...
    shell)
        ...
        docker-compose -f $DOCKER_COMPOSE_FILE -p $PROJECT_NAME exec app sh
        ;;
    db-shell)
        echo -e "${BLUE}Opening PostgreSQL shell...${NC}"
        docker-compose -f $DOCKER_COMPOSE_FILE -p $PROJECT_NAME exec postgres psql -U postgres -d Localkarya
        ;;
    clean)
        ...
        echo
        if [[ $REPLY =~ ^[Yy]$ ]]; then
            docker-compose -f $DOCKER_COMPOSE_FILE -p $PROJECT_NAME down -v --rmi all
